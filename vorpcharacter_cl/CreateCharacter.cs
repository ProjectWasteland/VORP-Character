...
namespace vorpcharacter_cl
{
    ...
    {
        //vars Scene
        private static bool isSelectSexActive;
        private static bool secondchance;
        public static bool isInCharCreation;
        public static string model_selected;
        private static readonly string model_f = "mp_female";
        private static readonly string model_m = "mp_male";
        private static int PedFemale;
        private static int charidx;
        private static dynamic usedcha;
        private static int PedMale;
        private static int Camera;
        private static int Camera_Male;
        private static int Camera_Female;
        private static int Camera_Editor;
        private static int Camera_FaceEditor;
        private static int Camera_Waist;
        private static int Camera_Legs;
        private static int Camera_Body;
        private static int Camera_Back;
        private static float DressHeading = 93.2f;
        ...
        public static void toggleOverlayChange(string name, int visibility, int tx_id, int tx_normal, int tx_material,
                                               int tx_color_type, float tx_opacity, int tx_unk, int palette_id,
                                               int palette_color_primary, int palette_color_secondary,
                                               int palette_color_tertiary, int var, float opacity)
        {
            for (var i = 0; i < SkinsUtils.overlay_all_layers.Count(); i++)
            {
                if (SkinsUtils.overlay_all_layers[i]["name"].ToString().Equals(name))
                {
                    ...
                    if (visibility != 0)
                    {
                        ...
                        else
                        {
                            ...
                            SkinsUtils.overlay_all_layers[i]["tx_id"] =
                                    (int)SkinsUtils.overlays_info[name][tx_id]["id"];
                        }
        ...
        public static async Task changeOverlays()
        {
            ...
            textureId = Function.Call<int>((Hash)0xC5E7204F322E49EB, texture_types["albedo"], texture_types["normal"],
                                           texture_types["material"]);

            foreach (var layer in SkinsUtils.overlay_all_layers)
            {
                if (layer["visibility"] != 0)
                {
                    int overlay_id = Function.Call<int>((Hash)0x86BB5FF45F193A02, textureId, layer["tx_id"],
                                                        layer["tx_normal"], layer["tx_material"],
                                                        layer["tx_color_type"], layer["tx_opacity"], layer["tx_unk"]);
                    if (layer["tx_color_type"] == 0)
                    {
                        Function.Call((Hash)0x1ED8588524AC9BE1, textureId, overlay_id, layer["palette"]);
                        Function.Call((Hash)0x2DF59FFE6FFD6044, textureId, overlay_id, layer["palette_color_primary"],
                                      layer["palette_color_secondary"], layer["palette_color_tertiary"]);
                    }
        ...
        public static void SetPlayerComponent(int _newIndex, uint category, string idlist, List<uint> male_components,
                                              List<uint> female_components)
        {
            var pPID = API.PlayerPedId();
            ...
            Function.Call((Hash)0xD3A7B003ED343FD9, API.PlayerPedId(), ConvertValue(skinPlayer["Hair"].ToString()),
                          true, true, true);
            Function.Call((Hash)0xD3A7B003ED343FD9, API.PlayerPedId(), ConvertValue(skinPlayer["Beard"].ToString()),
                          true, true, true);

            Function.Call((Hash)0xD3A7B003ED343FD9, API.PlayerPedId(), ConvertValue(skinPlayer["HeadType"].ToString()),
                          true, true, true);
            Function.Call((Hash)0xD3A7B003ED343FD9, API.PlayerPedId(), ConvertValue(skinPlayer["BodyType"].ToString()),
                          true, true, true);
            Function.Call((Hash)0xD3A7B003ED343FD9, API.PlayerPedId(), ConvertValue(skinPlayer["LegsType"].ToString()),
                          true, true, true);
        ...
        {
            var comp = "0x" + hex;
            var compInt = Convert.ToInt32(comp, 16);
        ...
        {
            TriggerEvent("vorpinputs:getInput", GetConfig.Langs["ButtonInputName"],
                         GetConfig.Langs["PlaceHolderInputName"], new Action<dynamic>(async cb =>
                         {
                             string result = cb;
                             await Delay(1000);
                             ...
                             {
                                 TriggerEvent("vorp:Tip", GetConfig.Langs["PlaceHolderInputName"],
                                              3000); // from client side
                                 SaveChanges();
                             }
                             else
                             {
                                 if (secondchance)
                                 {
                                     TriggerServerEvent("vorp_updateexisting", skinPlayer, clothesPlayer, result,
                                                        charidx, usedcha);
                                     StopCreation();
                                     StartAnim();
                                 }
        ...
        private static async void StartAnim()
        {
            ...
            var coords = new Vector3(GetConfig.Config["StartingCoords"][0].ToObject<float>(),
                                     GetConfig.Config["StartingCoords"][1].ToObject<float>(),
                                     GetConfig.Config["StartingCoords"][2].ToObject<float>());
        ...
        public static async void ApplyDefaultSkinCanaryEdition(int ped)
        {
            ...
            {
                var comp_body_male = "0x" + GetConfig.Config["Male"][0]["Body"][0];
                var comp_body_male_int = Convert.ToInt32(comp_body_male, 16);
                var comp_heads_male = "0x" + GetConfig.Config["Male"][0]["Heads"][0];
                var comp_heads_male_int = Convert.ToInt32(comp_heads_male, 16);
                var comp_legs_male = "0x" + GetConfig.Config["Male"][0]["Legs"][0];
                var comp_legs_male_int = Convert.ToInt32(comp_legs_male, 16);
            ...
            {
                var comp_body_female = "0x" + GetConfig.Config["Female"][0]["Body"][0];
                var comp_body_female_int = Convert.ToInt32(comp_body_female, 16);
                var comp_heads_female = "0x" + GetConfig.Config["Female"][0]["Heads"][0];
                var comp_heads_female_int = Convert.ToInt32(comp_heads_female, 16);
                var comp_legs_female = "0x" + GetConfig.Config["Female"][0]["Legs"][0];
                var comp_legs_female_int = Convert.ToInt32(comp_legs_female, 16);
        ...
        {
            Camera = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -560.83f, -3776.33f, 239.58f, -13.56231f, 0.00f,
                                             -91.93626f, 45.00f, false, 0);
            Camera_Male = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -559.6671f, -3775.44f, 239.4266f,
                                                  -9.622695f, 0.0f, -86.08074f, 45.00f, false, 0);
            Camera_Female = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -559.8455f, -3776.596f, 239.4435f,
                                                    -13.41718f, 0.0f, -88.04576f, 45.00f, false, 0);
            Camera_Editor = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -560.1333f, -3780.923f, 239.4437f,
                                                    -11.32719f, 0.0f, -90.96693f, 45.00f, false, 0);
            Camera_FaceEditor = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -558.9781f, -3780.955f, 239.186f,
                                                        6.591177f, 0.0f, -92.76723f, 40.00f, false, 0);
            Camera_Waist = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -559.1779f, -3780.964f, 238.4654f,
                                                   -0.6631846f, 0.0f, -91.76698f, 40.00f, false, 0);
            Camera_Legs = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -559.2103f, -3781.039f, 238.4678f,
                                                  -42.50001f, 0.0f, -89.2997f, 40.00f, false, 0);
            Camera_Body = API.CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", -560.6195f, -3780.708f, 239.1954f,
                                                  -15.75687f, 0.0f, -89.49976f, 40.00f, false, 0);
            ...
            var HashVeh = (uint)API.GetHashKey("hotAirBalloon01");
            await Miscellanea.LoadModel(HashVeh);

            var HashPed = (uint)API.GetHashKey("CS_balloonoperator");
        ...
        {
            var hash_f = (uint)API.GetHashKey(model_f);
            var hash_m = (uint)API.GetHashKey(model_m);
            /*
             ...
             */
            PedFemale = API.CreatePed(hash_f, -558.43f, -3776.65f, 237.7f, 93.2f, false, true, true, true);
            PedMale = API.CreatePed(hash_m, -558.52f, -3775.6f, 237.7f, 93.2f, false, true, true, true);
            /*
        ...
        private async void CreationSexPed(string model, int camedit)
        {
            ...
            var model_hash = (uint)API.GetHashKey(model);
        ...
        private async Task OnTick()
        {
            ...
            {
                await Miscellanea.DrawTxt(GetConfig.Langs["PressRightOrLeft"], 0.5f, 0.9f, 0.7f, 0.7f, 255, 255,
                                          255, 255, true, true);
            }

            if (isInCharCreation) //Fix Run Ped
            {
                await Miscellanea.DrawTxt(GetConfig.Langs["PressGuide"], 0.5f, 0.9f, 0.7f, 0.7f, 255, 255, 255,
                                          255, true, true);
